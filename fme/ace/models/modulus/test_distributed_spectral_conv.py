...
def setup_test():
    # 1. Check if we are running in a distributed environment (launched by torchrun)
    if "LOCAL_RANK" in os.environ and dist.is_available():
        ...
        if torch.cuda.is_available():
            if world_rank == 0:
                print("Running test on GPU using PyTorch Distributed")
            ...
            if world_rank == 0:
                print("Running test on CPU using PyTorch Distributed")
